...
$(document).ready(function(){
  ...
  <script>
	  ...
	  setTimeout( function() {  
		var TargetURL = "<?php echo Configure::read('base_url');?>stations/major_cfeature_change/"+clickedStation+"&delete_feature="+clickedStationKey+"-DN_XLH&customer_id=<?php echo $SELECTED_CNN; ?>";
$.fancybox.showLoading()  ;
...
?>
<?php $groupStatus = Configure :: read('groupStatus'); ?>
...
<?php if((isset($success)) && $success){?>
			...
			<div class="ok">
				<div class="message">
					...
					if(isset($_SESSION["group_created"])){
						...
						echo __($_SESSION["group_created"]).$_SESSION["success_groupCreated"]."<span class='width:10px;'></span>";
						
						unset($_SESSION["group_created"]);
						unset($_SESSION["success_groupCreated"]);
					}
  ...
  <h4><?php echo __("Group Detail") ?><?php echo $groupname?>
		...
		<div class="form-left" id="edit_stat_popupmenu" style="display: block">
          ...
          <div>
				<span  id="edit_stat" style="margin-left:15px; float:left;cursor:default;font-weight: bold;"<?php echo $readonly; ?>><?php __("GroupXlhEditOptions"); ?> </span>
				</div>
				<br>
	      				   <ul style="margin: 0 0 0 14px">
							<li><?php echo $html->link( __('changedGroup',true),array('div'=>false, 'controller'=>'features', 'action'=>'edit/dn_id:'.  $groupDetails['Group']['identifier'] . '&featureType=DN_XLH&spg=edit_xlh&lname=changedGroup'), array('class' => 'fancybox fancybox.ajax')); ?></li>
							
								<li><?php echo $html->link( __("Add Member", true),array('controller'=>'stations','action'=>'selectstation','group_id:'.$group_id .'&location_id='.$stationLocationid. '&type=single&memcount='.$groupMemberscount),array('class'=>'fancybox fancybox.ajax')); ?></li>
		...
		<div class="form-right">
					...
					#echo $form->input("identifier", array('type'=>'hidden','value'=>$groupDetails['Group']['identifier']));
                       if($groupDetails['Group']['sub_type']=="MLH"){
					   	echo __('groupMlhlhInfo_blurb');  
					   }                                        
                       else{
							echo __('groupDlhInfo_blurb');  
					   }                                    
				 ...
				 <tbody>
			       ...
			       <!-- <tr onmouseover="hoverRow(this,true);" onmouseout="hoverRow(this,false);">-->
					...
					<td>
							...
							echo __($groupArray['location'],true);
					...
					<td>
	                		<?php 
							echo __($groupStatus[$groupArray['status']],true);
							#echo $groupArray['status'];
					...
					?>tdlast">
						<div class="table-menu">
          				  <div class="table-menu-popup" style="z-index: 1">
          					  <ul>
          					  		<?php #for non-pilots only delete if there exists 
									if ($groupArray['station'] != '')
									{
									?>
									<li class="handset">
										<?php
									...
									<li class="delete">
										...
										$groupstationKeyId = $groupArray['stationkey_id'];
										$station_id = $groupArray['station'];
										#$sta_id = $group;
										
										#$groupstationKeyId = 'TEST1';
										#$station_id = 'TEST2';
										#$sta_id = 'TEST3';

										#For Pilot delete logic
										 if($stationkey_id != 1){
										if($groupArray['status'] != 7)
										{	if($j == 0)
											{
											#If there are no other members...
											//if ($groupMembers[1]['station'] == ''){
...
<div id="related-content">
        ...
        <div class="box">
        	 <h3><?php __('groupXlhEdit') ?></h3>
                 <p><?php __('groupXlhEdit_blurb') ?></p>
            ...
            <div style="display:none;" id="fullcont_type"  >
               <p  ><?php echo __('groupXlhEdit_helpText') ?></p>
