...
.table-content .table-right-ohne{
	...
	<?php 
	echo $form->create('Log',array('action'=>'adminreports','id'=>'filters','type'=>'GET')); ?>
	<br>
		...
		<div class="cb">
				...
				<table class="table-content phonekey">
					<thead>
						<tr class="table-top">
							...
							<th  class="table-column" style="width:63px;text-align: left;"><?php echo __("availableReports", true);?></th>
							<th class="table-right-ohne">&nbsp;</th>
						</tr>
						...
						<tr>
							<td class="table-column table-left-ohne">&nbsp;</td>
							<td>
							<select name="report" style="width:351px;"  onchange="javascript:submi_form('filters');">
								<!--  <option value="GLOBAL" <?php if ($customer_val == 'GLOBAL'){ echo ' SELECTED';}?>><?php __("Global Statistics") ?></option> -->
									<?php foreach($availReports  as $availReport){
										if($availReport[0]['reportName'] != '')
										{
											$additionalData = '(' . __('lastReported:',true) . $availReport['Log']['created'] . ',' . __('countReports:',true) . $availReport[0]['countReports'] . ')';
										}
										else {
											$additionalData = '';
										}
										?>
										<option value="<?php echo $availReport[0]['reportName'];?>"<?php if ($report == $availReport[0]['reportName']){ echo ' SELECTED';}?>><column><?php echo $availReport[0]['reportName'];?></column><column><?php echo $additionalData?></column></option>				
									 <?php }?>
									</select>
								</td>
							<td class="table-right-ohne">&nbsp;</td>
			...
			<h2><?php __('todayAdminReports') ?></h2>
				<div id="" class="table-content">
				...
				<table class="table-content phonekey" style="table-layout: fixed; width: 400px">
					...
					<td class="table-left">&nbsp;</td>
						<td class="table-left-ohne" style="width:250px;"><h5><?php __('reportTitle')?></h5></td>
		        		<td class="table-left-ohne" style="width:50px;"><h5><?php __('count')?></h5></td>
		        		<td class="table-left-ohne" style="width:150px;"><h5><?php __('date')?></h5></td>
	        		...
	        		<?php
	            		...
	            		?>
						<td class="table-left">&nbsp;</td>
						<td class="table-left-ohne" style="width:250px;"><?php echo $html->link($report[0]['title'],  array('controller'=> 'logs', 'action'=>'logdetails', $report['Log']['id']), array('class' => "fancybox fancybox.ajax",'style'=>$linkColor)); ?></td>
		        		<td class="table-left-ohne" style="width:50px;"><?php echo $report[0]['recordcount']; ?></td>
		        		<td class="table-left-ohne" style="width:150px;"><?php echo $report['Log']['created']; ?></td>
					...
					<h2><?php __('historyAdminReports') ?></h2>
				<div id="" class="table-content">
